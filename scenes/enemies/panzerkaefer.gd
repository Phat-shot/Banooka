...
func _baue() -> void:
	var panzer_mat := Materialbibliothek.metall(Farben.FELS.darkened(0.3))
	var chitin := Materialbibliothek.einfarbig(Farben.RINDE.darkened(0.35), 0.55)
	var naht_mat := Materialbibliothek.metall(Farben.KISTE_EISEN.lightened(0.1))
	var glut := Materialbibliothek.leuchtend(Farben.WARNUNG, 0.9)
	...
	for seite: float in [-1.0, 1.0]:
		for paar in 3:
			...
			huefte.name = "Huefte"
			huefte.position = Vector3(0.34 * seite, 0.28, -0.36 + float(paar) * 0.36)
			modell.add_child(huefte)
			_beine.append(huefte)

			_teil(huefte, _zylinder(0.05, 0.04, 0.32, 6), chitin,
					Vector3(0.14 * seite, -0.06, 0.0),
					Vector3(0.0, 0.0, 62.0 * seite), Vector3.ONE, "Schenkel")
			_teil(huefte, _zylinder(0.04, 0.025, 0.3, 6), chitin,
					Vector3(0.26 * seite, -0.2, 0.0),
					Vector3(0.0, 0.0, 14.0 * seite), Vector3.ONE, "Schiene")

	# --- Flacher, breiter Panzer ---
	_panzer = _teil(modell, _kugel(0.55, 16, 10), panzer_mat, Vector3(0.0, 0.34, 0.06),
			Vector3.ZERO, Vector3(1.0, 0.5, 1.28), "Panzer")
	# Mittelnaht und zwei Querstreben als Beschlag
	_teil(_panzer, _quader(Vector3(0.06, 0.06, 1.05)), naht_mat,
			Vector3(0.0, 0.5, 0.0), Vector3.ZERO, Vector3(1.0, 2.0, 1.0), "Naht")
	for streifen: float in [-0.3, 0.25]:
		_teil(_panzer, _quader(Vector3(0.95, 0.05, 0.1)), naht_mat,
				Vector3(0.0, 0.42, streifen), Vector3.ZERO, Vector3(1.0, 2.0, 1.0), "Strebe")

	# --- Kopf mit Zangen und Fühlern ---
	_kopf = Node3D.new()
	_kopf.name = "Kopf"
	_kopf.position = Vector3(0.0, 0.3, -0.62)
	modell.add_child(_kopf)

	_teil(_kopf, _kugel(0.26, 12, 8), panzer_mat, Vector3.ZERO,
			Vector3.ZERO, Vector3(0.9, 0.7, 0.8), "Schaedel")

	for seite: float in [-1.0, 1.0]:
		# Zangen nach vorn
		_teil(_kopf, _zylinder(0.06, 0.0, 0.36, 6), naht_mat,
				Vector3(0.11 * seite, -0.04, -0.2),
				Vector3(-78.0, 0.0, 18.0 * seite), Vector3.ONE, "Zange")
		# Glühende Augenpunkte
		_teil(_kopf, _kugel(0.06, 8, 6), glut, Vector3(0.14 * seite, 0.06, -0.12),
				Vector3.ZERO, Vector3.ONE, "Auge")
		# Fühler an eigenem Drehpunkt, damit sie wackeln können
		var wurzel := Node3D.new()
		wurzel.name = "Fuehlerwurzel"
		wurzel.position = Vector3(0.1 * seite, 0.14, -0.12)
		_kopf.add_child(wurzel)
		_fuehler.append(wurzel)
		_teil(wurzel, _zylinder(0.03, 0.015, 0.42, 6), chitin,
				Vector3(0.08 * seite, 0.18, -0.08),
...
func _animiere() -> void:
	...
	for i in _beine.size():
		...
		bein.position.y = 0.28 + maxf(schwung, 0.0) * 0.04

	if is_instance_valid(_panzer):
		_panzer.position.y = 0.34 + (sin(_phase * 2.0) * 0.02 if laeuft else 0.0)
